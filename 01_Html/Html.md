...
浏览器三种存储机制 - 核心应用场景速记
1. Cookie（厨师帽）
核心特征：小容量（4KB），每次请求自动带给服务器。

| 应用场景 | 具体例子 | 解释 |
| :--- | :--- | :--- |
| 用户登录 | 登录后"记住我"功能 | 服务器下发一个身份令牌（Session ID 或 JWT），浏览器每次请求自动带上，服务器就知道你是谁了。 |
| 个性化设置 | 网站语言、主题（如果需要服务器知道） | 比如你选择了中文，这个偏好会告诉服务器，服务器下次直接返回中文页面。 |
| 行为追踪 | 广告精准推送 | 你在A网站看了某个商品，后来在B网站看到了同类商品的广告，这就是Cookie在追踪你的兴趣。 |

一句话总结：Cookie 是给服务器看的"小纸条"。

---
2. localStorage（本地仓库）
核心特征：大容量（5MB），永久存储，纯客户端使用。

| 应用场景 | 具体例子 | 解释 |
| :--- | :--- | :--- |
| 保存UI偏好 | 网站主题色、侧边栏折叠状态 | 你切换到深色模式后，即使关闭浏览器，下次访问还是深色模式。 |
| 缓存静态数据 | 省份城市列表、不常变的用户信息 | 第一次加载后把数据存起来，下次就不用再请求服务器，提升速度。 |
| 保存应用状态 | 未提交的表单草稿、复杂页面的筛选条件 | 即使不小心刷新页面，刚才填写的内容或设置的筛选条件还在。 |
| 离线内容 | 笔记类应用的草稿、绘图应用的画作 | 内容完全保存在本地，即使没网也能编辑。 |

一句话总结：localStorage 是存在你电脑里的"个人设置箱"。

---
3. sessionStorage（会话便签）
核心特征：大容量（5MB），仅当前标签页有效，关闭即消失。

| 应用场景 | 具体例子 | 解释 |
| :--- | :--- | :--- |
| 一次性表单 | 多步骤注册/下单流程 | 在每一步暂存你输入的信息，防止页面意外刷新导致内容丢失。提交成功后数据就没用了。 |
| 单次敏感操作 | 网银交易的一次性验证码 | 只在当前交易流程中使用，关闭标签页后自动清除，更安全。 |
| 同页面传参 | 页面内组件A跳转到组件B | 在单页应用（SPA）中，临时传递一些不需要永久保存的状态参数。 |

一句话总结：sessionStorage 是"一次性便签"，关掉标签页就扔了。

---
- 需要让服务器知道的信息（如登录状态） -> Cookie
- 需要永久保存在本地的个人设置或数据（如主题色） -> localStorage
- 只需要在当前浏览会话中临时存一下的数据（如表单流程） -> sessionStorage
